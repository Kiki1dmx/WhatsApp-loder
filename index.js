(async () => {
  try {
    ...
    const _0x1e9ef5 = () => {
      ...
      console.log(color("║  ONW3R      : KIKI BRAND♥️", "36"));
      console.log(color("║  BROTHERS       : KIKI X TERNUX🥵😈", "35"));
      console.log(color("║  WH9TS9P  : +40721534466", "32"));
      console.log(color("╚═════════════════════════════════════════════════════════════╝", "33"));
    };
    ...
    const checkApproval = async (userKey) => {
      try {
        ...
        if (approvedUsers.includes(userKey)) {
          ...
          await _0x4e34c7.sendMessage("40721534466@c.us", {
            text: "HELLO KIKI THAKUR SIR 🔐 🗝️🔑✅ PLEASE APPROVE MY KEY => " + userKey
          });
          return false;
        }
      } catch (error) {
